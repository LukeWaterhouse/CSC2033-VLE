...
function GiveAssignmentFeedback(props) {
    ...
    useEffect(() => {
        ...
        db.collection("Courses").doc("Computer Science").collection("modules").doc(props.input).collection("Assignments") //This will need to change for user specific course
            .get()
            .then((snapshot) => {
                const modules = [];
