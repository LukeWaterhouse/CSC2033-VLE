      ...
      >
        <div className="panel-body" style={{ backgroundColor: "#616161" }}>
          <form onSubmit={sendMessage}>
            ...
            />
            <p style={{ visibility: showErrorMessage, color: "orangered" }}>
              Please input some text first!
            </p>
  ...
  return (
    ...
    >
      ...
      <h7
        style={
          AdminMessage
            ? { color: "green", marginBottom: "5px", fontWeight: "bold" }
            : { color: "white", marginBottom: "5px" }
        }
      >
        <u>{AdminMessage ? <p>{userName} (Admin)</p> : <p>{userName}</p>}</u>
      </h7>
      {text}{" "}
    </div>
  );
